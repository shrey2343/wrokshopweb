  ...
  return (
    <section className="py-20 px-4 bg-[#1A2940]/40">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
          ...
          >
            ...
            <ul className="space-y-3">
              {authority.leftItems.map((item, i) => (
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="bg-[#1A2940] rounded-2xl p-8 flex flex-col justify-center"
          >
            <h3 className="text-[#C9A84C] font-bold text-sm uppercase tracking-widest mb-4">
              {authority.rightTitle}
            </h3>
            <p className="text-white text-lg leading-relaxed italic">{authority.rightBody}</p>
          </motion.div>
        </div>
        ...
        >
          <p className="text-[#C9A84C] font-semibold text-lg">{authority.closing}</p>
        </motion.div>
      </div>
    </section>
  );
